  ...
  (lambda (b* b^* body* body*^ env type n)
    (conde
      ...
      ((fresh (x e e^ tx te rest rest^)
         ...
         (== `(vector ,tx ,n) te)
         (== `(((,x ,tx) (,e^ ,te)) . ,rest^) b^*)
         (infer-kernel rest rest^ body* body*^
           `((,x . ,tx) . ,env) type n))))))
  ...
  (lambda (stmt env envo rtype stmto)
    (fresh (e e^)
      (conde
        ...
        ((fresh (x start start^ end end^ stmt* stmt*^)
           (== stmt `(for (,x ,start ,end) . ,stmt*))
           (== stmto `(for (,x ,start^ ,end^) . ,stmt*^))
           (== env envo)
           (infer-expr start env 'int start^)
           (infer-expr end env 'int end^)
           (infer-stmts stmt* `((,x . int) . ,env) rtype stmt*^)))
        ((fresh (e e^ stmt* stmt*^)
           (== stmt `(while ,e . ,stmt*))
           (== stmto `(while ,e^ . ,stmt*^))
           (== env envo)
