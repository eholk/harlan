(library
  ...
  (define (walk-type t s)
    (match t
      ...
      ;; TODO: this is probably not the right thing to do with regions.
      ((fn ,r* (,[t*] ...) -> ,[t]) `(fn ,r* (,t* ...) -> ,t))
      ((fn (,[t*] ...) -> ,[t]) `(fn (,t* ...) -> ,t))
                ...
                (match `(,a* ,b*)
                  ...
                  (((,a ,a* ...) (,b ,b* ...))
                   (let ((s (unify-types a b s)))
                     (and s (loop a* b* s))))
                  (,else #f))))
             ;; These next three clauses are horriffic.
             (((fn ,r* (,a* ...) -> ,a) (fn (,b* ...) -> ,b))
              (let loop ((a* a*)
                         (b* b*)
                         (s s))
  ...
  (define (ground-decl d s)
    (match d
      ...
      ((fn ,name (,var ...)
       ...
       (match t
         ((fn [,r* ...] (,t* ...) -> ,t)
          `(fn ,name (,var ...) (fn ,(union r* region-params) (,t* ...) -> ,t)
                 (let-region ,(difference local-regions r*) ,body))))))))
