...
(define-mk (infer-stmt stmt env outr* letr stmto)
  (conde
    ...
    ((fresh (triv trivo)
       (== stmt `(print ,triv))
       (== stmto `(print ,trivo))
       (infer-triv triv env `() letr trivo)))
    ((fresh (triv op trivo opo)
       (== stmt `(print ,triv ,op))
       (== stmto `(print ,trivo ,opo))
       (infer-triv triv env `()  letr trivo)
       (infer-triv op env `()  letr opo)))
    ((fresh (triv trivo)
       (== stmt `(assert ,triv))
       (== stmto `(assert ,trivo))
       (infer-triv triv env `() letr trivo)))
    ...
    ((fresh (triv trivo)
       (conde
         ((== stmt `(return ,triv))
          (== stmto `(return ,trivo))
          (infer-triv triv env outr* letr trivo))
         ((== stmt `(return))
          (== stmto stmt)))))
    ((fresh (triv trivo r*)
       (== stmt `(do ,triv))
    ...
    ((fresh (x s so start end step
       ...
       (== envo `((,x () int) . ,env))
       (infer-triv start env `() letr starto)
       (infer-triv end env `() letr endo)
       (infer-triv step env `() letr stepo)
       (infer-stmt s envo outr* letr so)))
...
(define-mk (infer-lifted-expr expr env r* letr expro)
  (conde
    ((fresh (t triv trivo r to rest)
       (== expr `(make-vector ,t ,r ,triv))
       (== expro `(make-vector ,to ,trivo))
       (== r* `(,r . ,rest))
       (fill-type-with-region t rest to)
       (infer-triv triv env `() letr trivo)))
    ((infer-triv expr env r* letr expro))))

(define-mk (infer-triv triv env r* letr trivo)
  (conde
    ((fresh (c i u f b s)
       (conde
         ((== triv `(char ,c)))
    ...
    ((fresh (t c a to co ao)
       ...
       (== trivo `(if ,to ,co ,ao))
       (infer-triv t env `() letr to)
       (infer-triv c env r* letr co)
       (infer-triv a env r* letr ao)))
    ((fresh (rto rt argt argto fn-call
       ...
       (conde
         ((== fn-call `(var (,argt -> ,rt) ,fn))
          ...
          (infer-call-fn-args
           arg* argt argr* letr env argto arg*o)
    ...
    ((fresh (t v i to vo io r^)
       ...
       (fill-type-with-region t r* to)
       (infer-triv v env `(,r^ . ,r*) letr vo)
       (infer-triv i env `() letr io)))
    ((fresh (t to r*^)
       (== triv `(length ,t))
       (== trivo `(length ,to))
       (== r* `())
       (infer-triv t env r*^ letr to)))
    ((fresh (t to)
       (== triv `(addressof ,t))
       (== trivo `(addressof ,to))
       (infer-triv t env r* letr to)))
    ...
    ((fresh (op t1 t2 t1o t2o)
       ...
       (conde
         ...
         ((relop op)))
       (== r* `())
       (infer-triv t1 env `() letr t1o)
       (infer-triv t2 env `() letr t2o)))))
...
(define-mk (infer-call-fn-args
  ...
  (conde
    ...
    ((fresh (a d t dt r* ro to dto ao do)
       ...
       (== arg*o `(,ao . ,do))
       (fill-type-with-region t r* to)
       (fill-type-with-backup-region t letr to r*)
       (infer-triv a env r* letr ao)
       (infer-call-fn-args d dt ro letr env dto do)))))
...
(define-mk (infer-let-bindings b env letr bo envo)
  (conde
    ...
    ((fresh (x t e rest env^ resto eo to r*)
       ...
       (infer-lifted-expr e env r* letr eo)
       (infer-let-bindings
...
(define-mk (infer-kernel-dims d env letr do)
  (conde
    ((== d `()) (== do `()))
    ((fresh (dim d^ dimo d^o)
       (== d `(,dim . ,d^))
       (== do `(,dimo . ,d^o))
       (infer-triv dim env `() letr dimo)
       (infer-kernel-dims d^ env letr d^o)))))
