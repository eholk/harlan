...
(define (infer-module mod modo)
  ;; This is a necessary gensym, because cKanren does not know about
  ;; Harlan's gensym counter and could easily generate a non-unique
  ;; variable.
  (let ((region (gensym 'g_region)))
    (fresh (decls declso env)
      (== mod `(module . ,decls))
      (infer-initial-env decls env)
      (infer-decls decls env declso)
      (== modo `(module ,declso)))))
  ...
  (lambda (decls env)
    (conde
      ...
      ((fresh (extern/fn name r rest type env^)
         (== decls `((,extern/fn ,name . ,r) . ,rest))
         (== env `((,name . ,type) . ,env^))
         (infer-initial-env rest env^))))))

(define-mk infer-decls
  (lambda (decls env declso)
    (conde
      ((== '() decls) (== '() declso))
      ((fresh (decl declo rest resto name type)
         (== decls `(,decl . ,rest))
         (conde
           ((fresh (stmt stmto args arg-types typeo envo)
              (== decl `(fn ,name ,args ,type ,stmt))
              (== declo `(fn ,name ,args ,typeo ,stmto))
              (infer-fn-args args arg-types env envo)
              (infer-stmt stmt envo typeo stmto)))
           ((== decl `(extern ,name . ,type))
            (lookup env name type)
            (== declo decl)))
         (infer-decls rest env resto)
         (== declso `(,declo . ,resto)))))))
...
(define-mk infer-module
  (lambda (mod typed-mod)
    (fresh (decls decls^ envo)
      (== mod `(module . ,decls))
      (== typed-mod `(module . ,decls^))
      (infer-initial-env decls envo)
      (infer-decls decls decls^ envo))))
