(library
  ...
  (export infer-regions
    ...
    infer-triv
    infer-kernel-dims
    infer-kernel-bindings
    infer-kernel-free-vars
    memory-type
    scalar-type
    enforce-filled-regions
    extend-with-region
    fill-type-with-region
    fallback-region
    fallback-region-c)
  (import
    (rnrs)
  ...
  (define-mk (enforce-filled-regions x)
    (let loop ()
      (lambdag@ (a : s c)
        (let ((c (filter fallback-region-oc? c)))
          (cond
           ((null? c) (unitg a))
           (else
            ((fresh ()
  ...
  (define-mk (infer-stmt stmt env outr* letr stmto)
    (conde
      ...
      ((fresh (d b fv s so do bo fvo envo t)
         ...
         (infer-kernel-bindings b env bo envo)
  ...
  (define-mk (infer-kernel-bindings b env bo envo)
    (conde
      ((== b `())
       (== bo `())
       (== env envo))
      ((fresh (x xt e et i xto eo eto rest resto r r* env^)
         (== b `(((,x ,xt) (,e ,et) ,i) . ,rest))
         (== bo `(((,x ,xto) (,eo ,eto) ,i) . ,resto))
