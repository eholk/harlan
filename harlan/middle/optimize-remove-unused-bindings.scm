(library
  ...
  (trace-define-pass optimize-remove-unused-bindings : M7.1 (m) -> M7.1 ()
    ...
    (Expr
     ...
     ;;((for (,x ,[e0 x0] ,[e1 x1] ,[e2 x2]) ,[e x*])
     ;; (values `(for (,x ,e0 ,e1 ,e2) ,e)
     ;;         (union x0 x1 x2 (difference x* (set x)))))
     ((call ,[e x] ,[e* x*] ...)
      (values `(call ,e ,e* ...)
              (apply union x x*)))
     ((vector-ref ,t ,[e0 x0] ,[e1 x1])
      (values `(vector-ref ,t ,e0 ,e1)
              (union x0 x1)))
     ;;((if ,[e0 x0] ,[e1 x1])
     ;; (values `(if ,e0 ,e1) (union x0 x1)))
     ((if ,[e0 x0] ,[e1 x1] ,[e2 x2])
      (values `(if ,e0 ,e1 ,e2) (union x0 x1 x2)))
     ;; this set! should be a statement.
     ;;((set! ,[e0 x0] .,[e1 x1])
     ;; (values `(set! ,e0 ,e1) (union x0 x1)))
