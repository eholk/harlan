   ...
   (lambda (expr env type expro)
     (conde
       ...
       ((fresh (ve ie ve^ ie^)
          ...
          (infer-expr ve env `(vector ,type) ve^)))
       ((fresh (b* body* b^* body*^ env^)
          (== expr `(kernel ,b* . ,body*))
          (== expro `(kernel ,type ,b^* . ,body*^))
          (infer-kernel b* b^* body* body*^ env type))))))

(define infer-kernel
  (lambda (b* b^* body* body*^ env type)
    (conde
      ((== '() b*)
       (== '() b^*)
       (conde
         ...
         ((fresh (body body^ rest rest^ t env^)
            ...
            (infer-kernel b* b^* rest rest^ env^ type)))))
      ((fresh (x e e^ tx te rest rest^)
         (== `((,x ,e) . ,rest) b*)
         (infer-expr e env te e^)
         (== `(vector ,tx) te)
         (== `(((,x ,tx) (,e^ ,te)) . ,rest^) b^*)
         (infer-kernel rest rest^ body* body*^ `((,x . ,tx) . ,env) type))))))
